...
class DataTrainingArguments:
    ...
    input_column: str = field(
        default=None,
        metadata={"help": "Name of the column in the dataset to use as the input"}
    )
    output_column: str = field(
        default=None,
        metadata={"help": "Name of the column in the dataset to use as the expected output"}
    )
    ...
    re_labels: list[str] = field(
        default=None, metadata={"help": "How a relationship is indicated in the structured output. i.e. ['@CID@']."}
    )
    ner_labels: list[str] = field(
        default=None, metadata={"help": "How a named entity is indicated in the structerd output. i.e. ['@CHEMICAL@', '@DISEASE@']."}
    )
    coferent_matching_re: str = field(
        default="relaxed", metadata={"help": (
            "Wheter to use the coferent mentions to match relationships. Can be either 'relaxed', 'strict' or 'no'."
    ...
    coferent_matching_ner: str = field(
        default="relaxed", metadata={"help": (
    ...
    def __post_init__(self):
        ...
        # Also accepting csv files because my dataset is in csv format
        valid_extensions = ["json", "jsonl", "csv"]

        if self.train_file is not None:
            extension = self.train_file.split(".")[-1]
            assert extension in valid_extensions, f"`train_file` should be a file in one of the following formats: {valid_extensions}."
        if self.validation_file is not None:
            extension = self.validation_file.split(".")[-1]
            assert extension in valid_extensions, f"`validation_file` should be a file in one of the following formats: {valid_extensions}."
...
def main():
    ...
    # the sequence of dataclass here is the same as the sequence that get returned by .parse_*
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, training_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    elif len(sys.argv) == 2 and sys.argv[1].endswith(".yaml"): 
        # Lets do the same for a yaml file.
        model_args, data_args, training_args = parser.parse_yaml_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        # uses the default arguments or any arguments passed into the CLI.
    ...
    if data_args.source_prefix:
        logger.warning(
            f"You're using a prefix, this means that all input data will have the prefix {data_args.source_prefix}"
            "This is usefull if you're running a T5 model since it's pretrained with a prefix to indicate which"
    ...
    if data_args.dataset_name is not None: 
        ...
        if extension == "jsonl":
            ...
            builder_name = extension  # e.g. "parquet" or "csv"
    ...
    if len(tokenizer) > embedding_size:
        logger.warning(
            f"Token embedding size ({len(tokenizer)}) is bigger than the input embeddings ({embedding_size})."
            "Resizing model token embeddings to fit the tokenizer..."
        )
    ...
    def preprocess_function(examples, dataset_vars, padding, truncation, ignore_pad_token_for_loss):
        '''
        ...
        rel_column = data_args.output_column
    ...
    # Applying preprocessing to required datasets.
    if training_args.do_train:
    ...
    def compute_metrics(eval_preds, ner_labels, re_labels, coferent_matching_re, coferent_matching_ner):
        ...
        result.update(re_metric(predictions=decoded_preds, references=decoded_labels, ner_labels=data_args.ner_labels, re_labels=data_args.re_labels, coferent_matching=data_args.coferent_matching_re))
        result.update(ner_metric(predictions=decoded_preds, references=decoded_labels, ner_labels=data_args.ner_labels, re_labels=data_args.re_labels, coferent_matching=data_args.coferent_matching_ner))
    ...
    if training_args.do_predict:
        ...
        # Decode the output and save predictions
